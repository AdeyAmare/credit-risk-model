...
class TimeFeaturesExtractor(BaseEstimator, TransformerMixin):
    ...
    fit(X, y=None)
        Returns self.
    transform(X)
        Returns X with additional columns: txn_datetime, txn_hour, txn_day, txn_month, txn_year.
    """
...
class NumericAggregator(BaseEstimator, TransformerMixin):
    ...
    fit(X, y=None)
        Returns self.
    transform(X)
        Returns aggregated DataFrame with sum, mean, count, std per numeric column.
    """
...
class CategoricalTopKAggregator(BaseEstimator, TransformerMixin):
    ...
    -------
    fit(X, y=None)
        Determines top-K categories for each column.
    transform(X)
        Returns DataFrame of top-K category ratios per customer.
    """
...
class FeatureEngineeringPipeline(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> "FeatureEngineeringPipeline":
        ...
        self.cat_agg = CategoricalTopKAggregator(
            group_col=self.group_col, categorical_cols=self.categorical_cols, top_k=self.top_k
        )
